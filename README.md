...
**Last updated:** May 10, 2026 · **Officially Featured:** 5 · **Recommended Remote MCPs:** 12 · **Local Stdio MCPs:** 8 · **Categories:** 8 · **Version:** [1.0.0](CHANGELOG.md)
...
- ⭐ — Officially featured on LM Studio's [Popular MCP Integrations](https://lmstudio.ai/docs/integrations/mcp-remote/popular) page
- 🌐 — Remote MCP server (HTTP / SSE — works out of the box, no local install)
- 💻 — Local stdio MCP server (requires `npx` / `uvx` / Docker on your machine)
- 🔐 — OAuth 2.1 supported (one-click browser auth in LM Studio)
- 🪶 — Token-frugal — safe for local 7B – 30B models without context overflow
- 🅖 — Built or maintained by Google
- 🅜 — Built or maintained by the vendor (e.g., Notion-built Notion MCP)
- 🛠️ — Community-built
...
Curated by Element Labs on the [Popular MCP Integrations](https://lmstudio.ai/docs/integrations/mcp-remote/popular) page. All four are remote, OAuth-backed, and verified to work with LM Studio's transport.

## Project Management and Productivity

- ⭐ 🌐 🔐 🪶 🅜 [Linear](https://linear.app) - Create issues, search projects, update statuses, and more in Linear, directly from LM Studio. *MCP endpoint: `https://mcp.linear.app/mcp`. Use case: Issue triage and cycle planning during a coding session, surfacing in-flight work across projects.*
- ⭐ 🌐 🔐 🅜 [Notion](https://www.notion.so) - Search pages, create documents, and read from your Notion workspace. *MCP endpoint: `https://mcp.notion.com/mcp`. Use case: Pulling project context from Notion into a chat, drafting docs from research output. Heavier on tokens than Linear — use `allowed_tools` to scope.*
- ⭐ 🌐 🔐 🅜 [Atlassian (Jira + Confluence)](https://www.atlassian.com) - Work with Jira issues and Confluence pages from within LM Studio. *MCP endpoint: `https://mcp.atlassian.com/v1/mcp`. Use case: Issue triage, sprint context, drafting Confluence pages without leaving LM Studio.*

## Development and Observability

- ⭐ 🌐 🔐 🪶 🅜 [Sentry](https://sentry.io) - Query issues, inspect stack traces, and analyze errors from your Sentry projects. *MCP endpoint: `https://mcp.sentry.dev/mcp`. Use case: Triaging production errors with a local model, summarizing incident timelines, correlating issues with releases.*

---

# Recommended Remote MCP Servers

Beyond the official four, these remote MCP servers work with LM Studio's HTTP transport and are commonly used. Listed because they support OAuth 2.1 (one-click auth in LM Studio) or standard bearer auth without manual token management.

## Search and Knowledge

- 🌐 🪶 🅜 [Hugging Face](https://huggingface.co/mcp) - Hugging Face Hub access — model search, dataset retrieval, Spaces interaction. *MCP endpoint: `https://huggingface.co/mcp`. The example MCP in LM Studio's own docs. Use case: Discovering models for your local rig, looking up datasets, querying Hub metadata.*
...
- 🌐 🔐 🅜 [GitHub](https://github.com/github/github-mcp-server) - Official GitHub MCP server. *MCP endpoint via Copilot remote MCP or self-host. Use case: Repo search, PR/issue triage, code search across projects from a local-LLM session.*
- 🌐 🔐 🪶 🅜 [Cloudflare Developer Platform](https://developers.cloudflare.com/agents/model-context-protocol/) - Cloudflare Workers, Pages, R2, KV, D1. *Multiple MCP endpoints under `mcp.cloudflare.com/*`. Use case: Edge-platform debugging, deployment workflows, Workers config queries.*
- 🌐 🔐 🪶 🅜 [Vercel](https://vercel.com/docs/agents/mcp) - Vercel deployments and platform. *Use case: Deployment status, build debugging, project ops from chat.*
...
- 🌐 🔐 🪶 🅜 [PayPal](https://github.com/paypal/agent-toolkit) - PayPal merchant operations. *Use case: Transaction lookup, refund workflows, merchant account ops.*
- 🌐 🔐 🅜 [Square](https://developer.squareup.com/docs/sdks) - Square POS and payments. *Use case: Sales reporting, inventory queries, merchant operations from chat.*
...
- 🌐 🔐 🅜 [Figma](https://github.com/figma/mcp-server-guide) - Figma design files and components. *Use case: Reading frames into a coding session, generating components from designs.*
...
- 💻 🪶 🅜 [Filesystem MCP](https://github.com/modelcontextprotocol/servers/tree/main/src/filesystem) - Anthropic's reference filesystem MCP. *Use case: Read/write files in scoped directories from your local LLM. Foundational — most local workflows start here.*
- 💻 🪶 🅜 [Memory MCP](https://github.com/modelcontextprotocol/servers/tree/main/src/memory) - Anthropic's reference persistent-memory MCP. *Use case: Give your local LLM long-term knowledge that persists across sessions. Stores as JSON locally.*
...
- 💻 🅜 [Playwright MCP](https://github.com/microsoft/playwright-mcp) - Microsoft's official Playwright MCP. Referenced as the canonical local-stdio example in LM Studio's API docs. *Use case: Browser automation, web scraping, form filling, end-to-end testing — all driven by your local model.*
- 💻 🛠️ [Browserbase MCP](https://github.com/browserbase/mcp-server-browserbase) - Headless browser automation via Browserbase. *Use case: Cloud-backed browser automation when local Playwright is too heavy. Hybrid surface — local config, cloud execution.*
...
- [Awesome Gemini](https://github.com/rdmgator12/Gemini-Awesome-List-) — Sister directory tracking Gemini Connected Apps + CLI Extensions.
- [Awesome Mistral Connectors](https://github.com/rdmgator12/awesome-mistral-connectors) — Sister directory tracking Mistral Le Chat connectors.
- [Awesome MCP Servers](https://github.com/punkpeye/awesome-mcp-servers) — Broader directory of all known Model Context Protocol servers (the canonical reference for finding MCPs not yet on this curated list).
- [LM Studio docs — Use MCP Servers](https://lmstudio.ai/docs/app/mcp) — Official MCP integration guide.
- [LM Studio docs — Popular MCP Integrations](https://lmstudio.ai/docs/integrations/mcp-remote/popular) — Element Labs's curated directory.
- [LM Studio docs — Using MCP via API](https://lmstudio.ai/docs/developer/core/mcp) - Ephemeral + persistent MCP via the chat API.
- [LM Studio MCP launch blog](https://lmstudio.ai/blog/mcp) — June 2025 announcement.
- [LM Studio](https://lmstudio.ai) — Download the app.
